...
public class Jiafei extends AbstractIconicTraders {
    ...
    private static final Ingredient FOOD_ITEMS = Ingredient.of(
            FlopItems.JIAFEI_PRODUCT.get(), FlopItems.SLAGINIUM.get(), FlopItems.INFUSED_SLAGINIUM.get());
    ...
    protected SoundEvent getEatSound() {
        ...
        return switch (randVal) {
            case 1 -> FlopSounds.JIAFEI_EAT_1.get();
            case 2 -> FlopSounds.JIAFEI_EAT_2.get();
            default -> FlopSounds.JIAFEI_EAT_3.get();
        };
    }
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(10, new FlopcraftBreedingGoal(this, 1.0d));
    }
    ...
    public SoundEvent getAmbientSound() {
        if (ModUtil.isNewYears()) {
            return FlopSounds.JIAFEI_NEW_YEARS.get();
        }
        int hurtNum = this.random.nextInt(3);
        return switch(hurtNum) {
            case 1 -> FlopSounds.JIAFEI_1.get();
            case 2 -> FlopSounds.JIAFEI_2.get();
            default -> FlopSounds.JIAFEI_3.get();
        };
    }

    @Override
    protected SoundEvent getTradelessSound() {
        return FlopSounds.JIAFEI_PRODUCTLESS.get();
    }

    @Override
    public SoundEvent getHurtSound(DamageSource ds) {
        return FlopSounds.JIAFEI_HURT.get();
    }

    @Override
    public SoundEvent getDeathSound() {
        return this.random.nextBoolean() ? FlopSounds.JIAFEI_DEATH_1.get() : FlopSounds.JIAFEI_DEATH_2.get();
    }
    ...
    protected boolean itemIsFood(Item pItem) {
        return pItem.equals(FlopItems.JIAFEI_PRODUCT.get());
    }

    @Override
    protected boolean itemIsSpawnEgg(Item pItem) {
        return pItem.equals(FlopItems.JIAFEI_SPAWN_EGG.get());
    }
    ...
    protected boolean newYearsGifting(Player pPlayer) {
        if (!PlayerMethods.isNewgen(pPlayer)) {
            this.level().playSound(null, new BlockPos(this.getBlockX(), this.getBlockY(), this.getBlockZ()),
                    FlopSounds.JIAFEI_NEW_YEARS_TRADE.get(), SoundSource.AMBIENT);
        }
        return true;
    }
    ...
    public @NotNull SoundEvent getNotifyTradeSound() {
        if (ModUtil.isNewYears()) {
            return FlopSounds.JIAFEI_NEW_YEARS_TRADE.get();
        }
        int rand = this.random.nextInt(3);
        return switch(rand) {
            case 1 -> FlopSounds.JIAFEI_TRADE_1.get();
            case 2 -> FlopSounds.JIAFEI_TRADE_2.get();
            default -> FlopSounds.JIAFEI_TRADE_3.get();
        };
    }
    ...
    public void onBreeding(Level pLevel) {
        this.spawnAtLocation(FlopItems.JIAFEI_SEED.get());
